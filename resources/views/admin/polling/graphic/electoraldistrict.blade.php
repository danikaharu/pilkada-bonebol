                        ...
                        <form id="pollingForm">
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    ...
                                    <select id="electoraldistrict_id" name="electoraldistrict_id" class="form-select">
                                        <option disabled selected>-- Pilih Dapil --</option>
                                        @foreach ($electoraldistricts as $electoraldistrict)
                                            <option value="{{ $electoraldistrict->id }}">{{ $electoraldistrict->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                </div>
...
@push('script')
    ...
    <script>
        $('#viewGraphic').on('click', function(event) {
            ...
            var selectedElectoralDistrict = $('#electoraldistrict_id').val();
            $.ajax({
                url: "{{ route('admin.polling.graphicElectoralDistrict') }}",
                type: "GET",
                data: {
                    type: selectedType,
                    electoraldistrict_id: selectedElectoralDistrict
                },
